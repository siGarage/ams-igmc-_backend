...
export default {
    ...
    async createDepartment(req, res) {
        try {
            ...
            if (exist) {
                return res.status(403).send({ message: 'This department name is already exists!' });
            }
            let department = await DEPARTMENT.create(request);
            return res.status(201).send({department: department, message: "Department created successfully." });
        } catch (err) {
    ...
    async deleteDepartment(req, res) {
        try {
            ...
            return res.status(200).send({ id: id, message: "Department deleted successfully." })
        } catch (err) {
            return res.status(500).send({ message: "Internal Server Error" })
        }
    },
