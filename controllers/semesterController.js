...
export default {
    ...
    async createSemester(req, res) {
        try {
            ...
            if (Object.keys(request).length == 0) {
                return res.json(reply.failed("Number & course_id input is required."));
            }
            ...
            return res.status(201).send({ semester: semester, message: "Semester created successfully." });
        } catch (err) {
    ...
    async deleteSemester(req, res) {
        try {
            ...
            return res.status(200).send({ id: id, message: "Semester deleted successfully." })
        } catch (err) {
            return res.status(500).send({ message: "Internal Server Error" })
        }
    },
