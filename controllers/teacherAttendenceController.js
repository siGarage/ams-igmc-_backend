...
export default {
    ...
    async createTeacherAttendence(req, res) {
        try {
            ...
            let exist = await TEACHERATTENDENCE.findOne({ subject_id: request.subject_id, teacher_id: request.teacher_id, date: request.date });
            if (exist) {
                return res.status(403).send({ message: 'This teacher attendence is already exists!' });
            }
            let teacherAttendence = await TEACHERATTENDENCE.create(request);
            return res.status(201).send({ teacherAttendence: teacherAttendence, message: "Teacher Attendence created successfully" });
        } catch (err) {
    ...
    async deleteTeacherAttendence(req, res) {
        try {
            ...
            if (!teacherAttendence) {
                return res.status(404).send({ message: "Teacher Attendence not found." })
            }
            return res.status(204).send({ id: id, message: "Teacher Attendence deleted successfully." })
        } catch (err) {
            return res.status(500).send({ message: "Internal Server Error" })
        }
    },
    ...
    async updateTeacherAttendence(req, res) {
        try {
            ...
            return res.status(201).send({ message: "Teacher Attendence updated successfully" });
        } catch (err) {
            return res.status(500).send({ message: "Internal Server Error" });
        }
    },
